...
import (
	...
	"github.com/example/webapp-operator/test/utils"
)
...
var _ = Describe("Manager", Ordered, func() {
	...
	BeforeAll(func() {
		...
		Eventually(func() error {
			...
			return nil
		}, 3*time.Minute, 5*time.Second).Should(Succeed(), "Controller pods did not become ready")
	})
		...
		PIt("should ensure the metrics endpoint is serving metrics", func() {
			...
			verifyWebhookEndpointsReady := func(g Gomega) {
				cmd := exec.Command("kubectl", "get", "endpointslices.discovery.k8s.io", "-n", namespace,
					"-l", "kubernetes.io/service-name=k8s-operator-webhook-service",
